...
class Robot():
    ...
    def __init__(self, nameRobot, initX, initY):
        self.name = nameRobot
        self.posX = initX
        self.posY = initY
        self.goalX = initX
...
def task_allocation(robot1, robot2, robot3, robot4, robot5, stations, tasks_to_do ):
    ...
    for j in range(0, len(result[0])):
        if result[0][j] == 0:
            robot1.goalX = np.array(stations[tasks_to_do[result[1][j]]])[0]
            robot1.goalY = np.array(stations[tasks_to_do[result[1][j]]])[1]
            print('robot 1 is taking the task', j)
        elif result[0][j] == 1:
            robot2.goalX = np.array(stations[tasks_to_do[result[1][j]]])[0]
            robot2.goalY = np.array(stations[tasks_to_do[result[1][j]]])[1]
            print('robot 2 is taking the task', j)
        elif result[0][j] == 2:
            robot3.goalX = np.array(stations[tasks_to_do[result[1][j]]])[0]
            robot3.goalY = np.array(stations[tasks_to_do[result[1][j]]])[1]
            print('robot 3 is taking the task', j)
        elif result[0][j] == 3:
            robot4.goalX = np.array(stations[tasks_to_do[result[1][j]]])[0]
            robot4.goalY = np.array(stations[tasks_to_do[result[1][j]]])[1]
            print('robot 4 is taking the task', j)
        elif result[0][j] == 4:
            robot5.goalX = np.array(stations[tasks_to_do[result[1][j]]])[0]
            robot5.goalY = np.array(stations[tasks_to_do[result[1][j]]])[1]
...
def check_goal(robot, goals):
    if(robot.posX == robot.goalX and robot.posY == robot.goalY):
        print(robot.name, 'reached its goal')
        goals.remove((robot.goalX, robot.goalY))
...
if __name__ == '__main__':
    try:
        ...
        robot1 = Robot('robot1', 17, 1)
        robot2 = Robot('robot2', 19, 1)
        robot3 = Robot('robot3', 3, 5)
        robot4 = Robot('robot4', 17, 10)
        robot5 = Robot('robot5', 19, 10)
        ...
        # generate the tasks
        tasks_to_do = task_generator(number_station, stations)
        print('task generated')
        print(tasks_to_do)
        
        # task allocation
        goals = task_allocation(robot1, robot2, robot3, robot4, robot5, stations, tasks_to_do )
